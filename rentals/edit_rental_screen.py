...
class EditRentalScreen(Screen):
    def __init__(self, inventory_screen, associated_rental, **kwargs):
        ...
        self.layout.scroll_layout = GridLayout(
            cols=1,
            size_hint_y=None,
            size = (self.window_width, self.window_height * 0.375),
        ...
        rental = list(associated_rental.values())[0]
        for item_no in rental["items_rented"]:
            for row_widget in self.row_widgets.values():
                if row_widget.item_no == item_no:
                    row_widget.quantity_text_input.text = rental["items_rented"][item_no]["quantity"]
